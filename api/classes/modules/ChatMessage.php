...
class ChatMessage {
	...
	public function doGet($userId, $page, $itemsPerPage, $senderId, $receiverId, $year, $month) {
		$mysqli = $this->mysqli;
		...
		if ($receiverId == 0) {
			...
			$obj = $this->getMessageData($mysqli, $page, $itemsPerPage, $senderId, $receiverId, $year, $month);
		}
		$total = $obj->total;
		$data = $obj->data;
		$page = $obj->page;
		...
		$queryString = $this->getQueryString(array("module" => $module, "itemsPerPage" => $itemsPerPage, "senderId" => $senderId, "receiverId" => $receiverId, "year" => $year, "month" => $month));		
